...
def test_create_calendar_event_calls_calendar_api():
    fake_svc = MagicMock()
...
def test_draft_reply_creates_gmail_draft():
    fake_svc = MagicMock()
...
def test_send_rsvp_email_records_external_action():
    with patch("triage.gmail.send_email", return_value={"id": "msg-1"}) as send:
    ...
    assert actions[0]["gmail_message_id"] == "msg-1"


def test_send_rsvp_email_idempotent_skip_on_repeat():
    with patch("triage.gmail.send_email", return_value={"id": "msg-1"}):
...
def test_send_confirmation_email_calls_send():
